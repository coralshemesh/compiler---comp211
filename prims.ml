...
module Prims : PRIMS = struct
  ...
  let misc_ops =
    ...
    let all_car_cdr_funcs =
      ...
      let str_index = (string_of_int index) in
        ...
        mov rbx, PVAR(0)     ;PVAR returns the pair object on the stack - we can find it in rbp+ 4*8
        CAR rax, rbx
        ...
        set_car:
          ...
          mov rbx, PVAR(0)        ; get the pair - it's the first parameter on stack
          mov rcx, PVAR(1)        ; get the value we want to replace - it's the second parameter on the stack
          
          mov qword[rbx+TYPE_SIZE], rcx     ;insert to the car of the object the new value
        ...
        apply:
          ...
          mov r8, qword[rbp+8*3]        ; get num of args 
          add r8, 3                  
          shl r8, 3
  
          mov r8, qword[rbp + r8]             ;r8 gets the list
          mov r10, 0                    ;count how many elements there are in the list          
  
          loop_push_list_elements_to_stack_"^str_index^": \n
            cmp r8, SOB_NIL_ADDRESS             ;list in now empty
            je finish_push_list_elements_"^str_index^" \n
            CAR r9, r8          ;r9 gets the car list
            CDR r8, r8          ;r8 is the tail
            inc r10
            push r9 
            jmp loop_push_list_elements_to_stack_"^str_index^" \n
          
        finish_push_list_elements_"^str_index^": \n
          cmp r10, 0
          je finish_reverse_"^str_index^" \n
  
          mov r8, rsp               ;points to last element of the list which is first on stack
          mov r9, r10
          dec r9
          imul r9, r9, 8
          add r9, rsp               ;r9 points to first element of the list which is placed in [rsp+8*num_of_elements_in_list]
  
          reverse_"^str_index^": \n
            cmp r8, r9              
            jg finish_reverse_"^str_index^"\n
            mov r15, qword[r8]
            mov r14, qword[r9]
            mov qword[r8], r14
            mov qword[r9], r15
            sub r9, 8
            add r8, 8
            jmp reverse_"^str_index^"\n
  
          finish_reverse_"^str_index^": \n
            mov r8, qword[rbp+8*3]          ;r8 = num of args
            add r8, 2                       ;for env and n and proc - list 
            shl r8, 3
            add r8, rbp                     ;r8 now points to the last arg which is SOB
  
            mov r9, 4
            shl r9, 3
            add r9, rbp                     ;r9 now points to proc
  
            loop_push_args_again_"^str_index^": \n
              cmp r8, r9
              je finish_loop_push_args_again_"^str_index^"\n
  
              mov r11, qword [r8]
              
              push r11
  
              inc r10                       ;add to num of elements in list the num of args - calculate new n
              sub r8, 8
              jmp loop_push_args_again_"^str_index^" \n
  
            finish_loop_push_args_again_"^str_index^": \n
              mov rax, qword[r9]            ;rax = proc
              push r10                      ;push new num of args                      
  
        mov sil , byte[rax]                 ;same code as applic
        cmp sil, T_CLOSURE
        je contiue_apply_"^str_index^"\n
        
        send_error_apply_"^str_index^":\n
          jmp exit_apply_"^str_index^"\n
          
        contiue_apply_"^str_index^":\n
          CLOSURE_ENV r9, rax
          push r9
        ...
        apply_overwrite_stack_"^str_index^":\n
          mov r10, qword[r9]
          mov qword[r8], r10
  
          cmp r9, rsp
          je finish_apply_overwrite_stack_"^str_index^"\n
  
          sub r8 , 8 
          sub r9 , 8 
          jmp apply_overwrite_stack_"^str_index^"\n
        
        finish_apply_overwrite_stack_"^str_index^":\n 
          mov rsp , r8             ;insert new rsp to rsp
          pop rbp            ;insert old rbp to rbp
        ...
        push_nil_"^str_index^": \n
          push SOB_NIL_ADDRESS
          jmp finish_reverse_"^str_index^" \n
  
          exit_apply_"^str_index^":\n
          leave 
          ret \n
        "
  ...
  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops; all_car_cdr_funcs];;
end;;
